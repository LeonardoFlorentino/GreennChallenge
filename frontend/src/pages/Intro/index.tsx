...
export default function Intro() {
  ...
  useEffect(() => {
    ...
    const animate = (now: number) => {
      ...
      const maxRadius = orbitSize * maxRadiusRatio;
  ...
  useEffect(() => {
    const recalculateTechMarquee = () => {
      ...
      const copies = Math.max(4, Math.ceil((viewportWidth * 2) / baseWidth) + 2);
    ...
    <main className="relative min-h-screen overflow-hidden text-white">
      ...
      <section className="relative z-10 mx-auto flex w-full max-w-[2200px] flex-col px-4 py-10 sm:px-6 sm:py-12 lg:px-8 xl:px-10 2xl:px-12 min-[1800px]:max-w-none min-[1800px]:px-[4vw] min-[2200px]:px-[5vw]">
        <div className="w-full">
          <div className="grid gap-6 min-[1300px]:grid-cols-3 min-[1800px]:gap-8">
            <div className="rounded-2xl border border-white/20 bg-slate-950/40 p-5 backdrop-blur min-[1800px]:p-7">
              <div className="inline-flex items-center gap-3 rounded-full border border-white/20 bg-slate-950/40 px-4 py-2 backdrop-blur">
                <img src="/greenn-logo.ico" alt="Logo Greenn" className="h-5 w-5 rounded-sm object-cover" />
                <span className="text-sm uppercase tracking-[0.2em] text-emerald-200/85">Greenn Challenge</span>
              </div>
              ...
              <p className="mt-5 max-w-4xl text-base text-slate-200/85 sm:text-lg min-[1800px]:text-[1.45rem] min-[1800px]:leading-relaxed">
                Projeto técnico criado para investigar falhas no carrossel original e evoluir para uma solução full
                stack com foco em estabilidade e fluidez.
              </p>
            ...
            <div className="rounded-2xl border border-red-200/20 bg-slate-950/45 p-4 sm:p-5">
              <p className="text-sm font-semibold tracking-wide text-red-200/85">Vídeo</p>
              ...
              <div className="mx-auto mt-4 w-full rounded-xl border border-white/10 bg-black/40 p-2">
                <video className="aspect-video w-full rounded-lg object-contain" autoPlay muted loop playsInline preload="metadata">
                  <source src="/demos/greenn-carousel-duplicate-cards-and-swipe-lag-bug.mp4" type="video/mp4" />
            ...
            <div className="relative min-h-[320px] rounded-2xl border border-emerald-200/20 bg-slate-950/45 p-4 sm:p-5 min-[1300px]:min-h-[420px] min-[1800px]:min-h-[500px]">
              <p className="text-sm font-semibold tracking-wide text-emerald-200/85">Planetário</p>
              <div className="mt-4 flex w-full items-center justify-center min-[1300px]:h-[calc(100%-1.75rem)]">
                <div className="tech-orbit-panel relative flex h-[clamp(16rem,24vw,24rem)] w-[clamp(16rem,24vw,24rem)] items-center justify-center min-[1800px]:h-[clamp(22rem,19vw,30rem)] min-[1800px]:w-[clamp(22rem,19vw,30rem)]">
                  <div ref={orbitRef} className="tech-orbit relative h-full w-full rounded-full">
          ...
          <div className="mt-4 flex flex-wrap gap-2 text-xs min-[1800px]:mt-6 min-[1800px]:text-sm">
            <span className="rounded-full border border-white/15 bg-slate-950/40 px-3 py-1 text-slate-200/90">Diagnóstico de renderização</span>
            <span className="rounded-full border border-white/15 bg-slate-950/40 px-3 py-1 text-slate-200/90">Refatoração full stack</span>
            <span className="rounded-full border border-white/15 bg-slate-950/40 px-3 py-1 text-slate-200/90">Foco em performance</span>
          </div>

          <div className="mt-6 grid gap-6 min-[1300px]:grid-cols-2">
            <div className="rounded-2xl border border-emerald-200/20 bg-slate-950/45 p-4 sm:p-5">
              <p className="text-sm font-semibold tracking-wide text-emerald-200/85">Soluções implementadas</p>
            ...
            <div className="rounded-2xl border border-emerald-200/15 bg-emerald-950/25 p-4 sm:p-6">
              <p className="text-sm font-semibold tracking-wide text-emerald-200/85">Resultado alcançado até aqui</p>
              <ul className="mt-4 grid gap-2 text-[15px] leading-relaxed text-emerald-50/90">
                {achievements.map((item) => (
                  <li key={item} className="rounded-lg border border-white/10 bg-slate-950/20 px-3 py-2">
                    {item}
                  </li>
                ))}
              </ul>
          ...
          <div className="mt-6 rounded-2xl border border-emerald-200/15 bg-emerald-950/25 p-4 sm:p-6">
            <p className="text-sm font-semibold tracking-wide text-emerald-200/85">Tecnologias utilizadas</p>
            <div
              ref={techViewportRef}
              className="tech-marquee-shell mt-3 overflow-hidden rounded-xl border border-white/10 bg-slate-950/30 py-3"
            >
              <div className="tech-marquee-track flex w-max items-center" style={techMarqueeStyle}>
